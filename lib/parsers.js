...
function Parser(text) {
    ...
    this.parseExpression = function () {
        ...
        if (tryGetToken('==', TokenType.Operator))
            return expressions.equals(expr, parseTerm());

        if (tryGetToken('/=', TokenType.Operator))
            return expressions.notEquals(expr, parseTerm());
            
        if (tryGetToken('<', TokenType.Operator))
            return expressions.less(expr, parseTerm());
            
        if (tryGetToken('<=', TokenType.Operator))
            return expressions.lessEquals(expr, parseTerm());
            
        if (tryGetToken('>', TokenType.Operator))
            return expressions.greater(expr, parseTerm());
            
        if (tryGetToken('>=', TokenType.Operator))
            return expressions.greaterEquals(expr, parseTerm());
            
        if (tryGetToken('+', TokenType.Operator))
            return expressions.add(expr, parseTerm());

        if (tryGetToken('-', TokenType.Operator))
            return expressions.subtract(expr, parseTerm());

        if (tryGetToken('*', TokenType.Operator))
            return expressions.multiply(expr, parseTerm());

        if (tryGetToken('/', TokenType.Operator))
            return expressions.divide(expr, parseTerm());
